...
| [R1](#r1-draft-owned-a-class-residuals-registered-not-executed) | A class, registered | 9 packages after 024 | Owned by Draft specs; 024 executed the `subagent` and `tool-goal` halves and dropped the `lmo-pipeline-worker` row with its package |
...
| 5 | `packages/session/session-tags` | A | the durable tag registry and its whole package | 016 (Draft) - registered | 10 files, +487 |
| 6 | `packages/bundle/lmo-pipeline-worker` | A | an lmtech-only bundle placed in the official bundle tree | 018 (Draft) / 012 US3 - **absent from both trees**: the bundle no longer exists upstream or in the fork | 0 files |
...
| 9 | `packages/client/ui-tool` | B | no source row: the residual row is one `tagsBySession` fixture addition (session-tag compile coupling); the spec's toolview-priority reading is not backed by the measured diff ([B3](#b3-packagesclientui-tool)) | 020 FR-024 (B class) / 016 (fixtures) | 1 file, +1 |
...
| 16 | `packages/bundle/web-app` | pending - updated by 024 | one `@lingmeow.tech/dsh-session-tags` patch row plus the matching workspace dependency, with the removed pipeline-seam rows recorded in comments and a test asserting they are absent (T015) | 018 (Draft) - registered | 3 files, +59 |
| 17 | `packages/boot/app-boot` | pending - **converged by 024** | the `lmo-pipeline-worker` profile template is absent from both trees; the fork's only row is a test asserting the profile and its bundle stay out | 012 US3 / 018 FR-008 - converged | 1 file, +43 |
| 18 | `packages/extensions/tool-cordis` | pending | `src/api-catalog.ts` - lmtech entries written into the official catalog (entry-level ownership, see [R1](#r1-draft-owned-a-class-residuals-registered-not-executed)); 024 leaves the 016 entries for the catalog regeneration to drop | 016 / 007 - registered | 1 file, +84/-3 |
...
Decision: keep for now; each is a one-line test-fixture addition (`tagsBySession`) forced by the session-tag surface, which 024 re-landed as the `tagsBySession` projection on the session list snapshot. Delete together with spec 016. The inventory's `ui-locale` is really this `packages/client/locale`.
...
| `packages/session/session-tags` | 10 | 016 (Draft) | Migrate the session-tag business surface into the plugins repository; the bilingual README pair leaves with the package |
...
| `packages/boot/app-boot` | 1 | 012 US3 / 018 FR-008 | **Converged by 024**: the `lmo-pipeline-worker` profile template is absent from both trees; the residual row is a test asserting that absence |
| `packages/bundle/web-app` | 3 | 018 (Draft) | 024 keeps the `session-tags` row and its dependency; the remaining row convergence belongs to 018 |
| `packages/extensions/tool-cordis` | 1 | 016 / 007 | Entry-level ownership, measured 2026-09-12 at `+84/-3` (`src/api-catalog.ts`): the `lmoPipeline` `SERVICE_API` entry and its `Lmo*` type entries are already gone, the 007 entries (`SubagentRuntime.pendingQuestions`, the `DecisionAnswer*` types) disappear when the catalog is regenerated after the retirement, and only the 016 entries (`@Remote('tagsList'/'tagsSet'/'tagsRemove')` plus the `sessionTags` service) stay registered |
