...
| [P6](#p6-six-client-packages-with-fixture-coupling) | B class, compile coupling | `packages/client/{ui-workflow-run,ui-trajectory,ui-theme,ui-subagent,ui-jobs,locale}` | Keep the fixture field — it now couples to the `tagsBySession` projection 024 re-landed; delete together with spec 016 |
...
| 4 | `packages/api/session-controller` | A | lmtech session-control face (three `@Remote` tag verbs + the client `tagsBySession` projection) | 011 (Draft) - registered | 8 files, +225/-8 |
| 5 | `packages/session/session-tags` | A | the durable tag registry and its whole package | 016 (Draft) — **pending cleanup**: the fork copy is still in this tree | 10 files, +487 at the merge commit `af2a1e07ca`; re-measured 2026-09-17 at `83b9edf642` with `git diff --numstat c291e7961a HEAD -- packages/session/session-tags`: 9 files, +654 |
| 6 | `packages/bundle/lmo-pipeline-worker` | A | an lmtech-only bundle placed in the official bundle tree | 018 (Draft) / 012 US3 — **converged**, **absent from both trees**: the bundle no longer exists upstream or in the fork, and measured 2026-09-17 at `83b9edf642` `git ls-files packages/bundle/lmo-pipeline-worker` is empty | 0 files |
...
| 18 | `packages/extensions/tool-cordis` | pending | `src/api-catalog.ts` - lmtech entries written into the official catalog (entry-level ownership, see [R1](#r1-draft-owned-a-class-residuals-registered-not-executed)); 024 leaves the 016 entries for the catalog regeneration to drop | 016 / 007 - registered — **pending cleanup** for the 016 half: `src/api-catalog.ts` still carries `@Remote('tagsList'/'tagsSet'/'tagsRemove')` (lines 1467/1473/1479) and the `sessionTags` service entry (line 1959), measured 2026-09-17 at `83b9edf642` | 1 file, +84/-3 |
...
| 23 | `packages/test-support/client-runtime` | pending → B | session-tag no-op doubles, compile coupling ([P5](#p5-packagestest-supportclient-runtime)) | 020 keeps → **pending cleanup** with 016: `src/sessions.ts` still carries the `setSessionTags` / `removeSessionTags` doubles (measured 2026-09-17 at `83b9edf642`) | 1 file, +6 |
| 24 | `packages/client/ui-workflow-run` | pending → B | one-line test fixture `tagsBySession` ([P6](#p6-six-client-packages-with-fixture-coupling)) | 020 keeps → **pending cleanup** with 016: the fixture is still present (measured 2026-09-17 at `83b9edf642`) | 1 file, +1 |
| 25 | `packages/client/ui-trajectory` | pending → B | one-line test fixture `tagsBySession` ([P6](#p6-six-client-packages-with-fixture-coupling)) | 020 keeps → **pending cleanup** with 016: the fixture is still present (measured 2026-09-17 at `83b9edf642`) | 1 file, +1 |
| 26 | `packages/client/ui-theme` | pending → B | one-line test fixture `tagsBySession` ([P6](#p6-six-client-packages-with-fixture-coupling)) | 020 keeps → **pending cleanup** with 016: the fixture is still present (measured 2026-09-17 at `83b9edf642`) | 1 file, +1 |
| 27 | `packages/client/ui-subagent` | pending → B | one-line test fixture `tagsBySession` ([P6](#p6-six-client-packages-with-fixture-coupling)) | 020 keeps → **pending cleanup** with 016: the fixture is still present (measured 2026-09-17 at `83b9edf642`) | 1 file, +1 |
| 28 | `packages/client/ui-jobs` | pending → B | one-line test fixture `tagsBySession` ([P6](#p6-six-client-packages-with-fixture-coupling)) | 020 keeps → **pending cleanup** with 016: the fixture is still present (measured 2026-09-17 at `83b9edf642`) | 1 file, +1 |
| 29 | `packages/client/locale` | pending → B | one-line test fixture `tagsBySession`; the inventory's `ui-locale` is this package ([P6](#p6-six-client-packages-with-fixture-coupling)) | 020 keeps → **pending cleanup** with 016: the fixture is still present (measured 2026-09-17 at `83b9edf642`) | 1 file, +1 |
...
Decision: keep as B class. What the ledger and the gate's whitelist record is the class, and the residual row is compile coupling with no product behaviour that leaves together with spec 016. A genuine priority override would have to be re-evidenced before this row is restated as one.
...
Decision: keep for now, **pending cleanup** with spec 016 (session-tags migration). The file gains no-op doubles (`setSessionTags`, `removeSessionTags`) so the session-tag read/write surface compiles, and they are still there when measured on 2026-09-17 at `83b9edf642` (`src/sessions.ts` declares both, and the snapshot store constructor carries `tagsBySession`). The path the inventory used for this row, `packages/client/runtime`, is 017's package and holds no file in this tree (`git ls-files packages/client/runtime` is empty); that requirement moved to the plugins repository's `packages/dsh-lmtech-pipeline-client`, so this repository will not implement it, while the fork's `dev` line still tracks its 78 files — pending cleanup.
...
Decision: keep for now, **pending cleanup** with spec 016. Each is a one-line test-fixture addition (`tagsBySession`) forced by the session-tag surface, which 024 re-landed as the `tagsBySession` projection on the session list snapshot; measured 2026-09-17 at `83b9edf642`, all six are still present (`git grep -c tagsBySession` returns one hit in each of the six files listed above). The inventory's `ui-locale` is really this `packages/client/locale`.
...
One consequence needed repair beyond the generated artifacts: `packages/bundle/lmo-pipeline-worker/tests/bundle.spec.ts` still asserted both dropped rows in the patch list and in the worker config, so the package's test lane (`vitest run packages/bundle/lmo-pipeline-worker/tests/bundle.spec.ts`) failed with two "must mount" assertions after the convergence. The expectations now carry the removal with the owning spec inline, and the lane is green again. What the removal leaves open is a functional question for spec 018: the worker profile no longer mounts any `ctx.lmoPipeline` seam, because the pre-rehoming package names resolve nowhere — 018 owns re-adding that row under the rehomed plugin name. **Converged**: measured 2026-09-17 at `83b9edf642`, that lane left with the package — `packages/bundle/lmo-pipeline-worker` tracks 0 files in this tree, so what 018 converges here is the bundle's absence from both trees, and what stays pending is the web-app bundle's `session-tags` row and dependency (`cordis.patch.yml:80-81`, `package.json:42`).

## R1 Draft-owned A-class residuals: registered, not executed

Spec 020 executes only the A-class face whose owning spec is Approved (`015`); 024 then executes the parts its own spec owns. Every other A-class package belongs to a Draft spec and is registered back to the PM - the fork keeps its diff until the owning spec lands. Measured status on 2026-09-12 in the 024 worktree against `c291e7961a`:

| Package | Files | Owning spec (status) | Registered action |
| --- | --- | --- | --- |
| `packages/api/session-controller` | 8 | 011 (Draft) | Remove the reverse dependency / RPC closure on `dsh-session-tags`; 024 keeps the three `@Remote` tag verbs and the client `tagsBySession` projection |
| `packages/session/session-tags` | 10 | 016 (Draft) — **pending cleanup** | Migrate the session-tag business surface into the plugins repository; the bilingual README pair leaves with the package. Re-measured 2026-09-17 at `83b9edf642`, the package is still in this tree: 9 files, +654 against `c291e7961a` |
| `packages/subagent/subagent` | 0 | 007 (Draft) + 013 US4 | **Converged by 024**: `decision-answer`, `pendingQuestions`, and the `list-children` ordering flip are removed; the rationale is consolidated in the [retirement Agent Note](../.agents/notes/implemented/simplification/2026-09-12-subagent-decision-answer-retirement.md) |
...
| `packages/bundle/web-app` | 3 | 018 (Draft) — **change pending** | 024 keeps the `session-tags` row and its dependency; the remaining row convergence belongs to 018. Measured 2026-09-17 at `83b9edf642`, both are still in place: `cordis.patch.yml:80-81` (`id: session-tags`, `name: '@lingmeow.tech/dsh-session-tags'`) and `package.json:42` (`"@lingmeow.tech/dsh-session-tags": "workspace:^"`) |
| `packages/extensions/tool-cordis` | 1 | 016 / 007 — **pending cleanup** (016 half) | Entry-level ownership, measured 2026-09-12 at `+84/-3` (`src/api-catalog.ts`): the `lmoPipeline` `SERVICE_API` entry and its `Lmo*` type entries are already gone, the 007 entries (`SubagentRuntime.pendingQuestions`, the `DecisionAnswer*` types) disappear when the catalog is regenerated after the retirement, and only the 016 entries (`@Remote('tagsList'/'tagsSet'/'tagsRemove')` plus the `sessionTags` service) stay registered — still present when measured 2026-09-17 at `83b9edf642` (lines 1467/1473/1479 and 1959) |
