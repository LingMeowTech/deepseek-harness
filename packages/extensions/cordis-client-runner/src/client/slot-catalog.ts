...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.input.hindsight',
    kind: 'single',
    scope: 'session',
    summary: 'The named Hindsight-memory seat in the composer tool row, immediately right of the plan seat and beside the access-mode control — one occupant, so taking it means rendering the whole memory enable toggle plus bank multi-select yourself.',
    doc: 'The named Hindsight-memory seat in the composer tool row, immediately\nright of the plan seat and beside the access-mode control — one\noccupant, so taking it means rendering the whole memory enable toggle\nplus bank multi-select yourself. Same `locked`-only owner share and the\nsame renders-nothing-while-empty contract as the plan seat.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.input.hindsight\', () => ctx.slots.register(\n      { name: \'conversation.input.hindsight\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  {
    key: 'settings.plugin.inventory.item',
    kind: 'keyed',
    scope: 'root',
    summary: 'One extra row inside the plugin inventory list (keyed by plugin id).',
    doc: 'One extra row inside the plugin inventory list (keyed by plugin id).',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'settings.plugins.tab\' (client-ui-settings-plugin-inventory), so it exists while that entry is mounted',
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'settings.plugin.inventory.item\', () => ctx.slots.register(\n      { name: \'settings.plugin.inventory.item\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-settings-plugin-inventory/src/client/slot-contract.ts:15',
  },
  {
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The pipeline browsing region, rendered below `sidebar.workspaces`: the lmo pipeline hierarchy (project → pipeline → state → job), pipeline sessions, and the approve/rerun actions.',
    doc: 'The pipeline browsing region, rendered below `sidebar.workspaces`: the\nlmo pipeline hierarchy (project → pipeline → state → job), pipeline\nsessions, and the approve/rerun actions. Declared by this package\'s\n\'sidebar\' entry; ui-lmo-pipeline registers the browser.',
    registerOptions: [],
    ownerProps: [
