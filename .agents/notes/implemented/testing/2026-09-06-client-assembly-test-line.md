...
**`remoteDefaultResponses` holds default state for the Remote endpoints the roster reads.** The table lists the endpoints the `web` roster calls while booting and rendering with no sessions, no workspaces, and default settings, plus the auxiliary per-session reads a surface triggers once its first row appears (for example the `session/tagsList` pull the session list issues per row), with each row commented with its caller. A spec layers its own `RemoteTable` on top; an endpoint the roster reaches without a default fails the spec at `dispose()`.
